...
- 工作进度改为非线性 workflow reader，内置科研、VC 会议纪要、VC 行业研究 / 投决三类预设，并支持在顶部创建自定义工作线；
...
例如科研人格加载科学问题、实验约束、证据状态和科研工作流；投资人格加载项目材料、行业资料、投决规则和 VC 工作流。动态路由先识别 Domain，再生成最小上下文包：当前目标、当前任务、必要记忆、最近证据、预算和待裁决项。离开 Domain 时卸载其正文，只保留可验证的状态引用，避免总管上下文持续膨胀。
