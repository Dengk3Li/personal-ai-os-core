...
- 工作进度改为非线性 workflow reader，能够点击 Loop、并行分支和汇合节点；
- 节点详情显示模型、执行适配器、Attempt、心跳和产物事件；
- 工作台直接显示 9 项已分配、3 项运行中、2 项待验收、4 项已收口和 4 次重复运行；
...
|---|---|---|
| 模块地图、工作流切换、任务节点选择 | 浏览器内读取匿名合成投影，可查看 Loop、分支、分配和事件 | 保留交互；数据改为读取本地运行时投影 |
