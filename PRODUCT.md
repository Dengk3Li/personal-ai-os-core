...
- 模块详情将结构上下游、反馈关系、外部输入、内部处理、主要输出和接口协议分开呈现。
- “运行模块”视图继续从 `personal-ai-os.module/v1` manifest 解析能力和依赖，支持拖动、平移、缩放、视口适配与上下游聚焦；选中模块的批注可以进入当前工作流成为修正任务。
...
- 过往对话和结果的自动采集、候选提取与冲突核验仍是规划能力。公开内核只提供候选登记、人工审核与按任务加载，不会自动推断人格或自动提升个人模型。
...
- 当前全量测试通过 173 个 Python 用例和 60 个 Workbench 用例，覆盖工作方式候选校验、审核主体与追加事件、规则数量和上下文预算、主体与领域隔离、显式任务加载、模块—任务关系来源保护、公开系统模块身份与私人模块匿名化、模块进度联动和模块批注交接，以及持久目标、跨工作线续推、累计步数与 Token、预算受限、显式完成验收、续推竞争与崩溃恢复门、Domain/工作线双层分页、单线路聚焦、执行模式就绪判断、递归系统图与下钻、工作流结构校验和求值、逐任务路由、有界自动推进、Human Gate、计划同步、持久化恢复、模型上下文隔离、依赖产物接续、跨 RuntimeStore 派发竞争、异常脱敏和静态回退。
