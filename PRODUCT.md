...
- 模型、自动路由、执行适配器和 API 状态集中在后台“设置”；工作进度不提供逐任务配置控件。API 密钥只由服务端环境变量持有。
...
- 当前全量测试通过 181 个 Python 用例和 66 个 Workbench 用例，覆盖工作协议加载、凭据拒绝、缺失门与作用域隔离、会议协议、已确认习惯自动注入、任务后经验复核、后台固定执行器绑定、自动路由任务启动、计划幂等同步、递归子图布局与跨层模块边界，以及既有的工作方式候选、模块—任务关系、持久目标、双层分页、非线性工作流、有界自动推进、Human Gate、运行恢复、隐私投影和低频配置入口。
