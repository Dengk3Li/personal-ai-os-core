...
![长期工作的四视图工作空间](docs/assets/workbench.jpg)
...
| 研究脉络 | 从同一任务状态投影研究问题、证据阶段、主张综合和结论核验；执行中的材料与已接受证据保持清晰区分。 |
| 待我决定 | 集中处理计划确认与 Human Gate。批准后任务继续分派，退回后保持阻塞。 |

科研任务仍在工作进度中执行；研究脉络提供领域化的证据视角，不建立第二套任务事实。界面沿用 Cognitive Intake 的纸张底色、墨绿、荧光信号、衬线标题和双栏卡片语言。
...
| 四视图投影 | 从同一任务状态生成只读系统地图、工作进度、研究脉络和待裁决队列。 |
...
当前版本为 `0.4.0` 私有预览。公开前需要选择许可证；仓库目前没有授予复制、修改或再分发代码的许可。
