...
![Personal AI OS 工作流阅读器](docs/assets/workbench.jpg)
...
- 循环验证流包含三个 Loop 和多个并行分支；
- 并行生产流包含分叉、独立执行和汇合；
- 系统构建流通过可替换的能力模块完成装配。

其中 9 项已分配，3 项正在运行，2 项待验收，4 项已收口，另有 4 次重复运行保留在轨迹中。点击任一节点，可以查看模型、执行适配器、Attempt 次数、心跳和产物事件。
...
| 入口 | 负责什么 |
|---|---|
| 模块地图 | 查看核心模块、可选模块、能力接口、可用状态和可替换插槽。 |
...
仓库目前没有授予复用许可。[许可证方案](docs/LICENSING_OPTIONS.zh-CN.md)建议采用双许可证：个人和非商业用途免费，商业用途单独付费并署名。
